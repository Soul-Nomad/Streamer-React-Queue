...
const TWITCH_COLORS = [
  '#FF0000', '#1E90FF', '#00FF00', '#B22222', '#FF7F50',
  '#9ACD32', '#FF4500', '#2E8B57', '#DAA520', '#D2691E',
  '#5F9EA0', '#FF69B4', '#8A2BE2', '#00FF7F', '#A855F7'
];
...
export default function Lobby() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'live-queue' | 'gaming' | 'just-chatting'>('all');
  ...
  const [twitchFollowedData, setTwitchFollowedData] = useState<{ online: any[]; followed: any[] } | null>(null);
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session?.provider_token) {
        ...
        if (sUser) {
          localStorage.setItem(`twitch_provider_token_${sUser.id}`, session.provider_token);
        }
      } else if (sUser) {
        const savedToken = localStorage.getItem(`twitch_provider_token_${sUser.id}`);
        if (savedToken) {
  ...
  const parseSupabaseUser = (sUser: SupabaseUser) => {
    ...
    const preferredUsername = meta.custom_claims?.preferred_username || meta.preferred_username || meta.name || meta.full_name || 'user';
    ...
    const hash = preferredUsername.split('').reduce((acc: number, char: string) => acc + char.charCodeAt(0), 0);
  ...
  useEffect(() => {
    ...
    fetch(`${getBackendUrl()}/api/twitch/followed?token=${providerToken}&userId=${twitchUserIdState}`)
      ...
      .catch(err => {
         console.error('Failed to sync followed streaming channels:', err);
         setLoadingTwitchData(false);
      });
  ...
  useEffect(() => {
    if (supabaseUser && twitchUsername && twitchDisplayName) {
      ...
      if (targetRoom) {
        ...
        const isPastHost = targetRoom === activeRoom && localStorage.getItem('active_role') === 'host';
        ...
        saveVisitHistory(targetRoom, isPastHost ? 'Host' : payload.displayName, payload.avatarUrl);
  ...
  const handleLoginTwitch = async () => {
    await supabase.auth.signInWithOAuth({
      provider: 'twitch',
      options: {
        scopes: 'moderator:read:followers channel:read:subscriptions user:read:follows user:read:subscriptions moderation:read',
  ...
  const buildTwitchPayload = (asBroadcaster = false) => {
    ...
    return {
      ...
      followedAt: asBroadcaster ? new Date(Date.now() - 1000 * 60 * 60 * 24 * 365).toISOString() : undefined,
      color: twitchColor,
      badges
    };
  };
  ...
  const handleCreate = async () => {
    ...
    const twitchId = supabaseUser.identities?.find(i => i.provider === 'twitch')?.id || payload.login;
    
    try {
      const { data: insertedRoom, error } = await supabase.from('rooms').upsert({
      ...
      console.warn('PostgreSQL rooms registry schema was not fully loaded, using socket runtime room creation instead.');
    }

    localStorage.setItem('host_join_template', JSON.stringify({
      name: payload.displayName,
      userId: supabaseUser.id,
      twitchData: payload
    }));
  ...
  const handleJoin = (targetRoomId: string) => {
    ...
    saveVisitHistory(cleanRoomCode, matchedRoom?.hostName || 'Streamer Pool', matchedRoom?.hostAvatar || '');
  ...
  const processedOnlineStreamers = useMemo(() => {
    ...
    if (twitchFollowedData && twitchFollowedData.online && twitchFollowedData.online.length > 0) {
      twitchFollowedData.online.forEach(stream => {
        ...
        const activeRoom = discoveredRooms.find(r => r.hostLogin?.toLowerCase() === stream.user_login?.toLowerCase() || r.hostTwitchUserId === stream.user_id);
  ...
  const offlineFollowedStreamers = useMemo(() => {
    ...
    if (twitchFollowedData && twitchFollowedData.followed && twitchFollowedData.followed.length > 0) {
      ...
      twitchFollowedData.followed.forEach((follow: any) => {
         ...
         const isOnline = twitchFollowedData.online?.some((o: any) => o.user_id === follow.broadcaster_id);
         if (!isOnline) {
            list.push({
  ...
  const activeQueuesStats = useMemo(() => {
    return {
      ...
      totalVideosInQueues: discoveredRooms.reduce((acc, r) => acc + r.queueCount, 0)
    };
  }, [discoveredRooms]);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#070708]">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-xs font-mono text-[#B0B0B0] uppercase tracking-wider animate-pulse">Carregando Plataforma...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#0a0a0c] text-white flex flex-col font-sans relative antialiased selection:bg-[#9146FF]/30">
      ...
      <header className="sticky top-0 z-[100] h-16 bg-[#0f0f13]/95 border-b border-[#1b1b22] backdrop-blur-md px-4 sm:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 select-none">
          <div className="w-9 h-9 bg-gradient-to-br from-[#9146FF] to-[#FF6B35] rounded-md flex items-center justify-center shadow-lg shadow-[#9146FF]/15">
          ...
          <div className="text-left hidden sm:block">
            ...
            <p className="text-[10px] text-[#8e8e9c] font-semibold mt-0.5">Sincronização de Fila Twitch em Tempo Real</p>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-sm sm:max-w-md mx-4 relative">
          ...
          <input 
            ...
            className="w-full h-9 bg-[#16161f] border border-[#2d2d3a] hover:border-[#424254] focus:border-[#9146FF] rounded-lg px-3 pl-9 text-xs text-white placeholder-[#52526b] focus:outline-none focus:ring-1 focus:ring-[#9146FF]/50 transition-all font-sans"
          />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {supabaseUser && (
            <div className="flex items-center gap-2">
              <button 
                ...
                className="h-9 px-3 bg-[#1c1c24] border border-[#2e2e3d] hover:bg-[#252530] text-xs font-bold text-white rounded-lg flex items-center gap-1.5 transition-all cursor-pointer"
              ...
              <button 
                ...
                className="h-9 px-3 bg-gradient-to-r from-[#9146FF] to-[#7c3aed] hover:from-[#772ce8] hover:to-[#6d28d9] text-xs font-extrabold text-white rounded-lg flex items-center gap-1.5 transition-all cursor-pointer shadow-md shadow-[#9146FF]/15"
          ...
          {!supabaseUser ? (
            <button 
              ...
              className="h-9 px-4 bg-[#9146FF] hover:bg-[#772ce8] text-xs font-extrabold text-white rounded-lg flex items-center gap-2 tracking-wide uppercase transition-all cursor-pointer shadow-lg shadow-[#9146FF]/20"
              id="header_login_twitch"
            >
              <Twitch className="w-4 h-4 fill-current" /> Entrar com Twitch
            </button>
          ) : (
            <div className="flex items-center gap-2 bg-[#16161f] border border-[#252532] p-1.5 px-2.5 rounded-lg">
              {twitchAvatar ? (
                <img 
                  src={twitchAvatar}
                  alt={twitchDisplayName}
                  referrerPolicy="no-referrer"
                  className="w-6 h-6 rounded-md border border-[#303042]"
                />
              ) : (
                <div 
              ...
              <div className="text-left hidden lg:block leading-none mr-2">
                <span className="text-[10px] font-bold text-slate-200 block truncate" style={{ color: twitchColor }}>{twitchDisplayName}</span>
                <span className="text-[8px] text-slate-500 font-mono block">@{twitchUsername.toLowerCase()}</span>
              </div>

              <button 
                onClick={handleSignOut}
                title="Desconectar Conta"
                className="p-1 text-slate-500 hover:text-red-400 hover:bg-slate-800/20 rounded transition-colors cursor-pointer"
              >
      ...
      {(isSecretKeyMistake || isMissingConfig) && (
        <div className="bg-[#2a1215] border-y border-[#FF3B30]/20 py-2.5 px-6 text-xs text-slate-300 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <p>
              <strong>Atenção Desenvolvedor:</strong> {isSecretKeyMistake 
          ...
          <a href="https://supabase.com" target="_blank" rel="noreferrer" className="text-[10px] font-black underline uppercase text-white hover:text-[#9146FF]">Ajustar Chave</a>
        </div>
      )}
      ...
      <div className="flex-1 flex max-w-[1600px] w-full mx-auto relative">
        ...
        <aside className="w-64 bg-[#0a0a0c] border-r border-[#15151c] p-4 hidden lg:flex flex-col gap-5 shrink-0 select-none">
          ...
          <div className="p-4 bg-gradient-to-br from-[#12121a] to-[#0d0d12] border border-[#20202d] rounded-xl space-y-3">
            <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest font-black block">Status da Rede</span>
            
            <div className="grid grid-cols-2 gap-2 text-left">
              <div className="bg-[#191924]/60 p-2 rounded-lg border border-[#292937]/50">
                <span className="text-[8px] text-slate-400 font-bold block uppercase leading-none">Salas Ativas</span>
                <span className="text-sm font-extrabold text-slate-100 block mt-1 font-mono">{activeQueuesStats.totalRooms}</span>
              </div>
              <div className="bg-[#191924]/60 p-2 rounded-lg border border-[#292937]/50">
                <span className="text-[8px] text-slate-400 font-bold block uppercase leading-none">Fila Total</span>
                <span className="text-sm font-extrabold text-slate-100 block mt-1 font-mono">{activeQueuesStats.totalVideosInQueues}</span>
              </div>
            </div>
            ...
            <div className="border-t border-[#1a1a24] pt-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                  <FolderHeart className="w-3.5 h-3.5 text-red-400" /> Canais Offline
                </span>
                <span className="text-[9px] text-[#A0A0A0] font-mono">{offlineFollowedStreamers.length}</span>
              </div>

              <div className="space-y-2 overflow-y-auto max-h-40 pr-1">
                {offlineFollowedStreamers.slice(0, 3).map((streamer, idx) => (
                  <div key={idx} className="flex items-center justify-between p-1 rounded hover:bg-slate-900/40">
                    <div className="flex items-center gap-2">
                      <img src={streamer.avatarUrl} className="w-6 h-6 rounded-md object-cover border border-[#1b1b22] opacity-60" alt="" />
                      <span className="text-xs text-slate-400 truncate max-w-[100px] font-bold block leading-none">{streamer.displayName}</span>
                    </div>
                    <span className="text-[8px] font-mono text-slate-600 block shrink-0">Offline</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto max-w-full min-w-0 space-y-10 pb-20">
          ...
          <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-[#170e28] via-[#100c1e] to-[#07070a] border border-[#231a38] p-6 sm:p-10 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_right,rgba(145,70,255,0.08)_0%,transparent_60%)] pointer-events-none" />
            <div className="absolute top-3 right-3 bg-[#9146FF]/15 border border-[#9146FF]/35 text-[#cda8ff] px-2.5 py-1 rounded-full text-[9px] font-mono uppercase tracking-widest font-black animate-pulse flex items-center gap-1.5">
              <Radio className="w-3 h-3 text-[#10B981] shrink-0" /> Spotlight Room
            </div>

            {/* Banner Meta Info */}
            <div className="text-left space-y-4 max-w-xl">
              <div className="flex items-center gap-2">
                <span className="bg-[#9146FF]/10 text-[#9146FF] border border-[#9146FF]/30 px-3 py-1 rounded-md text-[10px] font-mono uppercase font-black">Mídia Livre</span>
                <span className="text-[11px] text-[#A0A0A0] flex items-center gap-1 font-semibold"><Flame className="w-4 h-4 text-orange-500" /> Sala Em Destaque de Hoje</span>
              </div>
              ...
              <p className="text-sm text-slate-400 font-sans leading-relaxed">
                Nossa plataforma processa uploads de vídeos curtos como Reels, TikToks e YouTube Shorts em tempo real sem latência do player, permitindo moderação coletiva. Conecte de forma simples!
              </p>
              ...
              <div className="pt-2 flex flex-wrap gap-3 items-center">
                {supabaseUser ? (
                  <button 
                    ...
                    className="h-10 px-5 bg-gradient-to-r from-[#FF6B35] to-[#FF8C42] hover:scale-[1.02] active:scale-95 text-xs text-white font-black tracking-wider uppercase rounded-xl transition-all flex items-center gap-2 cursor-pointer shadow-lg shadow-[#FF6B35]/25"
                  >
                    <Sparkles className="w-4 h-4" /> 
                    {discoveredRooms.length > 0 ? 'Participar da Fila Maior' : 'Iniciar Fila do Meu Canal'}
                  </button>
                ) : (
                  <button 
                    onClick={handleLoginTwitch}
                    className="h-10 px-5 bg-[#9146FF] hover:bg-[#772ce8] text-xs text-white font-black tracking-wider uppercase rounded-xl transition-all flex items-center gap-2 cursor-pointer shadow-lg shadow-[#9146FF]/25"
                  >
                    <Twitch className="w-4 h-4 fill-current" /> Começar Agora via Twitch
                  </button>
                )}

                <button 
                  onClick={() => setIsJoinModalOpen(true)}
                  className="h-10 px-4 bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.08] text-xs font-bold text-slate-300 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
                >
                  <Search className="w-4 h-4" /> Buscar outra sala
                </button>
              </div>
            </div>
            ...
            <div className="w-full sm:w-auto relative shrink-0">
              <div className="w-full sm:w-44 bg-[#14121a] border border-[#2b253b] p-4 rounded-2xl text-center space-y-3 relative shadow-2xl">
                <div className="w-14 h-14 bg-gradient-to-br from-[#FF6B35] to-[#9146FF] rounded-xl flex items-center justify-center mx-auto text-white shadow-lg">
                  <Tv className="w-7 h-7" />
                </div>
                <div className="space-y-1">
                  <span className="text-xs font-black block font-mono text-slate-100">Fila Compartilhada</span>
                  <span className="text-[10px] text-slate-500 font-sans block leading-tight">Espectadores enviam links de vídeos, streamers assistem ao vivo.</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#1b1b22] pb-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                <div className="w-1.5 h-1.5 bg-white rounded-full animate-ping" /> Filas Ativas no App
              </button>
              <button 
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs bg-slate-800 text-slate-400 font-mono font-bold px-2 py-0.5 rounded">
                  {processedOnlineStreamers.filter(s => s.roomId !== null).length} Live
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                {recentHistoryList.map((hist, idx) => (
                  ...
                  >
                    ...
                    {hist.hostAvatar ? (
                      <img src={hist.hostAvatar} className="w-9 h-9 rounded-lg object-cover border border-slate-700/30" alt="" />
                    ) : (
                    ...
                    <div className="min-w-0 flex-1 leading-tight">
                      <span className="text-[9px] text-[#FF8C42] font-mono uppercase tracking-widest font-black leading-none block">Recente • Código {hist.roomId}</span>
                      <span className="text-xs font-black text-slate-100 block truncate mt-1">{hist.hostName}</span>
                      <span className="text-[9px] text-slate-500 font-mono block mt-1">Conectado hà {new Date(hist.visitedAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-indigo-400 font-bold font-mono">Matches baseados em tags</span>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {processedOnlineStreamers.slice(0, 3).map((stream, idx) => {
                ...
                return (
                  <div 
                    key={idx}
                    className="group bg-[#111116] border border-[#1f1f2a] hover:border-indigo-500/20 rounded-xl overflow-hidden transition-all duration-300 hover:scale-[1.01] text-left flex flex-col justify-between"
                  >
                    <div className="p-4 space-y-4">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-3">
                          <img src={stream.avatarUrl} className="w-8 h-8 rounded-lg object-cover border border-[#2d2d3c]" alt="" />
                          <div>
                            <span className="text-xs font-black block" style={{ color: stream.color }}>{stream.displayName}</span>
                            <span className="text-[9px] text-indigo-400 font-mono font-bold block uppercase mt-0.5">{matchPercentages[idx % 4]}% Compatibilidade</span>
                          </div>
                        </div>
                        <span className="text-[9px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-2 py-0.5 rounded-full font-bold">RECOMENDADO</span>
                      </div>

                      <p className="text-xs text-slate-300 leading-relaxed">
                        Sugerido para você porque você assiste e segue canais da categoria <strong className="text-indigo-300">"{stream.game || 'Just Chatting'}"</strong>.
                      </p>

                      <div className="bg-[#151520] p-2 rounded-lg border border-indigo-950/10 flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0" />
                        <span className="text-[10px] text-slate-400 truncate block font-sans">Título: <em>"{stream.title || 'Sem título'}"...</em></span>
                      </div>
                    </div>

                    <div className="bg-[#14141d] p-3 px-4 border-t border-[#1b1b26] flex items-center justify-between">
                      <span className="text-[10px] text-slate-500 font-mono font-bold uppercase block">{stream.viewers.toLocaleString('pt') || '1k'} assistindo</span>
                      {stream.roomId ? (
                        <button 
                          onClick={() => handleJoin(stream.roomId)}
                          className="h-8 px-3 bg-indigo-600 hover:bg-indigo-500 text-[10px] text-white font-extrabold uppercase tracking-wider rounded-lg transition-all"
                        >
                          Conectar à fila
                        </button>
                      ) : (
                        <button 
                          ...
                          className={`h-8 px-3 text-[10px] uppercase font-bold rounded-lg transition-all ${
                            requestedQueues.includes(stream.login)
                              ? 'bg-slate-800 text-slate-500'
                              : 'bg-white/[0.03] hover:bg-[#9146FF]/10 text-slate-300 hover:text-white border border-white/[0.08]'
                          }`}
                        >
                          {requestedQueues.includes(stream.login) ? 'Fila Solicitada!' : 'Solicitar Fila'}
                        </button>
                      )}
                    </div>
                  </div>
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs bg-[#FF6B35]/10 text-[#FF6B35] px-2 py-0.5 rounded-full font-mono font-bold animate-pulse">VIBRANTE AGORA</span>
            </div>
            ...
            {discoveredRooms.length === 0 ? (
              <div className="p-10 bg-[#121217] border border-white/[0.03] rounded-2xl text-center space-y-3">
                ...
                <div className="max-w-md mx-auto space-y-2">
                  <span className="text-sm font-bold text-slate-300 block">Nenhuma Fila Ativa em Andamento</span>
                  <p className="text-xs text-slate-500 leading-relaxed pb-2">Seja o primeiro a iniciar uma rede de vídeo na plataforma criando sua própria sala!</p>
                  <button 
                    onClick={() => setIsHostConfirmOpen(true)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {discoveredRooms.map((room) => (
                  <div 
                    key={room.roomId}
                    className="bg-[#121219] border border-[#2d2038]/50 hover:border-[#FF8C42]/50 p-5 rounded-xl text-left flex flex-col justify-between transition-all group"
                  >
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          {room.hostAvatar ? (
                            <img src={room.hostAvatar} className="w-9 h-9 rounded-lg shrink-0 object-cover border border-[#2d2d3c]" alt="" />
                          ) : (
                            <div className="w-9 h-9 rounded-lg bg-slate-800 flex items-center justify-center text-xs font-bold shrink-0">
                              {room.hostName.substring(0,2).toUpperCase()}
                            </div>
                          )}
                          <div>
                            <span className="text-xs font-black text-slate-100 block group-hover:text-[#FF8C42] transition-colors">{room.hostName}</span>
                            <span className="text-[10px] text-slate-500 font-mono block mt-0.5">Streamer Host</span>
                          </div>
                        </div>
                        <div className="bg-[#FF8C42]/10 border border-[#FF8C42]/25 px-2.5 py-1 rounded-lg text-center shrink-0">
                          <span className="text-[8px] text-slate-400 uppercase tracking-wider block font-bold leading-none">Canal</span>
                          <span className="text-xs font-mono font-black text-[#FF8C42] block mt-1">{room.roomId}</span>
                        </div>
                      </div>

                      {/* Stats bento indicators */}
                      <div className="grid grid-cols-2 gap-3 pt-1">
                        <div className="bg-[#1f1a26]/40 p-3 rounded-lg border border-[#2a2333]/30">
                          <span className="text-[9px] text-[#A0A0A0] font-mono uppercase block">Conectados</span>
                          <span className="text-base font-black text-slate-200 block mt-1 font-mono">{room.usersCount}</span>
                        </div>
                        <div className="bg-[#1f1a26]/40 p-3 rounded-lg border border-[#2a2333]/30">
                          <span className="text-[9px] text-[#A0A0A0] font-mono uppercase block">Vídeos na Fila</span>
                          <span className="text-base font-black text-[#10B981] block mt-1 font-mono">{room.queueCount}</span>
                        </div>
                      </div>
                    ...
                    <div className="border-t border-[#23202e] mt-4 pt-4 flex items-center justify-between">
                      ...
                      <button 
                        ...
                        className="h-8 px-4 bg-gradient-to-r from-[#FF8C42] to-[#ff9e5e] hover:scale-[1.02] text-xs text-white font-extrabold uppercase rounded-lg transition-all cursor-pointer flex items-center gap-1"
                      >
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500 hover:text-slate-300 transition-colors">Mais ativos</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {processedOnlineStreamers.slice(2, 6).map((stream, idx) => {
                const growthRates = ['+94% atividade', '+41% envios', '+32% chat', '+18% fila'];
                return (
                  <div key={idx} className="bg-[#111116] border border-[#1b1b24] p-4 rounded-xl text-left flex items-center justify-between transition-all hover:border-[#9146FF]/25">
                    <div className="flex items-center gap-3 min-w-0">
                      <img src={stream.avatarUrl} className="w-8 h-8 rounded-lg object-cover border border-[#23232d]" alt="" />
                      <div className="min-w-0">
                        <span className="text-xs font-black text-slate-200 block truncate" style={{ color: stream.color }}>{stream.displayName}</span>
                        <span className="text-[10px] text-amber-400 font-bold block mt-0.5">{growthRates[idx % 4]}</span>
                      </div>
                    </div>

                    <button 
                      onClick={() => stream.roomId ? handleJoin(stream.roomId) : handleRequestQueue(stream.login)}
                      className="text-[9px] font-extrabold uppercase tracking-wider bg-white/[0.02] hover:bg-[#9146FF]/10 text-slate-300 hover:text-white border border-white/[0.08] px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer"
                    >
                      {stream.roomId ? 'Entrar' : 'Seguir'}
                    </button>
                  </div>
                );
              })}
      ...
      <footer className="bg-[#0f0f13] border-t border-[#1b1b22] py-6 px-8 text-center text-xs text-slate-500 font-sans mt-auto leading-relaxed">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="font-bold text-[#FF8C42]">Aviso: Sistema em Desenvolvimento</p>
          <div className="flex gap-4 text-[11px] font-bold">
            <button onClick={() => window.dispatchEvent(new CustomEvent('openModal', { detail: 'termos' }))} className="text-slate-400 hover:text-white cursor-pointer transition-colors">Termos de Uso</button>
            <button onClick={() => window.dispatchEvent(new CustomEvent('openModal', { detail: 'privacidade' }))} className="text-slate-400 hover:text-white cursor-pointer transition-colors">Política de Privacidade</button>
          </div>
        </div>
      </footer>
      ...
      <AnimatePresence>
        {isJoinModalOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-black uppercase text-slate-500 font-mono tracking-widest">Acesso de Espectador</span>
                <button onClick={() => setIsJoinModalOpen(false)} className="text-slate-500 hover:text-white transition-colors cursor-pointer text-xs">Fechar</button>
              </div>

              <div className="space-y-1">
                <h4 className="text-base font-black uppercase text-slate-100">Conectar à Sala de Fila</h4>
                <p className="text-[11px] text-slate-400 leading-relaxed">Digite o código de 4 letras compartilhado pelo streamer ou moderadores do canal.</p>
              </div>
      ...
      <AnimatePresence>
        {isHostConfirmOpen && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-black uppercase text-slate-500 font-mono tracking-widest">Painel do Streamer</span>
                <button onClick={() => setIsHostConfirmOpen(false)} className="text-slate-500 hover:text-white transition-colors cursor-pointer text-xs">Fechar</button>
              </div>

              <div className="space-y-1">
                <h4 className="text-base font-black uppercase text-slate-100">Iniciar Sessão de Broadcaster</h4>
                <p className="text-[11px] text-slate-400 leading-relaxed">Você será o proprietário exclusivo e host administrador desta sala de fila de mídias.</p>
              </div>
              ...
              <div className="p-3 bg-[#171720]/80 border border-[#22222d] rounded-xl flex items-center gap-3">
                ...
                <div className="text-left font-sans">
                  <span className="text-xs font-bold text-slate-300 block">Identidade broadacster:</span>
                  <span className="text-xs font-black block text-[#9146FF]">@{twitchUsername.toLowerCase()}</span>
                </div>
              </div>
