...
export async function createSession(roomId: string, hostId: string, twitchData: any) {
  ...
  const initialSessionState = {
    ...
    users: existingSettingsJson?.users || [],
    queue: existingSettingsJson?.queue || [],
    currentVideoId: existingSettingsJson?.currentVideoId || null,
    history: existingSettingsJson?.history || [],
    isPlaying: existingSettingsJson?.isPlaying || false,
    currentTime: existingSettingsJson?.currentTime || 0,
